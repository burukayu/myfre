...
function Home() {
  ...
  const getUsers = () => {
    fetch(`http://localhost:3200/employee/employee` )
      ...
      .then(data => {
    setEmployees(data);
    // Log the first letter of each employee's full name
    data.forEach(emp => {
      console.log(emp.fullname?.charAt(0) || '', "← first letter");
    });
  })
...
// }
  ...
  <img
    ...
    alt={employee.fullname?.charAt(0) || ''}
  ...
  >
    {employee.fullname?.charAt(0).toUpperCase() || ''}
          ...
          <div className="ms-2">
            <h1 className="h5 mb-1">{employee.fullname}</h1>
